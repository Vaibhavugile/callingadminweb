...
const createdMs = (call) => {
  ...
  const c =
    call.createdAt || data.createdAt || data.ts || data.timestamp || null;
  if (!c) return null;
  if (c.seconds)
    return (
      c.seconds * 1000 +
      (c.nanoseconds ? Math.floor(c.nanoseconds / 1000000) : 0)
    );
...
function callsToCsvRows(callsArray) {
  ...
  for (const c of callsArray) {
    ...
    const nextFollowUpIso = nextFollowUpRaw
      ? fmtTimestamp(nextFollowUpRaw)
      : "";
    const nextFollowUpLocal = nextFollowUpRaw
...
export default function CallsPage() {
  ...
  // - if tenantFilter === "all" → all connected
  // - else only selected tenant
  const allowedTenantIdsForHook = useMemo(() => {
    if (!connectedTenantIds.length) {
      return ["__NO_TENANT__"]; // block everything until profile is loaded
    }
    if (tenantFilter === "all") return connectedTenantIds;
    return [tenantFilter];
  }, [connectedTenantIds, tenantFilter]);

  // calls hook with tenant restriction
  const {
    calls = [],
    loading,
    error,
  } = useCalls({ pageSize: 1200, allowedTenantIds: allowedTenantIdsForHook });

  // only show connected tenants in dropdown
  ...
  const filtered = useMemo(() => {
    ...
    return calls.filter((c) => {
      ...
      // missed / rejected use matchCallFilter (direction + duration + finalOutcome)
      if (
        (filter === "missed" || filter === "rejected") &&
        !matchCallFilter(data, filter)
      ) {
        return false;
      }
      ...
      if (range.startMs !== null && (cMs === null || cMs < range.startMs)) {
        return false;
      }
      if (range.endMs !== null && (cMs === null || cMs > range.endMs)) {
        return false;
      }
  ...
  // CSV export: from currently filtered rows
  const exportCsv = () => {
    try {
      ...
      <div className="calls-inner">
        ...
        {error && (
          <div className="calls-error">
            Error loading calls: {String(error)}
          </div>
        )}
          ...
          {loading ? (
            ...
            filtered.map((c) => {
              ...
              const nextFollowUpRaw =
                lead.nextFollowUp ?? data.nextFollowUp ?? null;
              ...
              return (
                <article
                  ...
                  tabIndex={0}
                  onKeyDown={(e) => {
                    if (e.key === "Enter") toggleExpand(c.id);
                  }}
                >
                  ...
                  <div className="call-right">
                    <div className="call-tenant">
                      Tenant:{" "}
                      <span className="muted-strong">{tenant}</span>
                    </div>
                    ...
                    <div className="call-expanded">
                      <div className="expanded-grid">
                        ...
                        <div>
                          ...
                          <div className="expanded-value">
                            {lead.eventDate
                              ? lead.eventDate.seconds
                                ? new Date(
                                    lead.eventDate.seconds * 1000
                                  ).toLocaleDateString()
                                : new Date(
                                    lead.eventDate
                                  ).toLocaleDateString()
